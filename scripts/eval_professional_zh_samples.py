"""Run baseline detection on the professional_zh bootstrap corpus."""
...
DEFAULT_INPUT = Path("dataset/seeds/professional_zh/seed_samples_v1.jsonl")
DEFAULT_OUTPUT = Path("reports/professional_zh_seed_baseline_v1.json")
...
def evaluate(input_path: Path, output_path: Path) -> None:
    ...
    table = Table(title="professional_zh baseline")
    ...
    for domain, s in sorted(summary.items()):
