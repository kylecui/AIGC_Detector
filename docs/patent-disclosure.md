...
- 中文仲裁块（ZH Arbitration Block）：当统计阶段判定为"人写"但编码器阶段p_ai ≥ 0.35时，判定统计阶段结果可信度不足，采用编码器阶段的输出作为最终结果；
...
过采样前的基线模型在HC3问答数据上训练，对正式教材文体的检测能力不足（p_ai=0.0096）。过采样的贡献在于：用极少量目标领域数据（82条训练样本×10倍=820条）使模型学会识别该文体，同时保持对原有领域知识的记忆。
...
流程图展示了10倍过采样持续学习的完整数据流：HC3-Chinese数据集（10,510样本）与企业AI安全教材（98个文本块）经8:1:1拆分后，对教材训练样本进行10倍过采样（82→820），合并为11,330样本的训练集，进行LoRA微调训练。在9个不重复hold-out测试样本上达到100%召回率，在6个完整章节文件上同样达到100%泛化召回率，HC3原领域准确率保持97.0%。
