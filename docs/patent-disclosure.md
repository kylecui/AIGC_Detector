...
在教材文本领域适配任务上，10倍过采样持续学习的效果显著：

| 指标 | 过采样前（v1） | 过采样后（v2） | 提升幅度 |
|------|:-------------:|:-------------:|:-------:|
| 教材章节p_ai | 0.0096 ❌ | **0.9980** ✅ | +0.9884 |
| 教材召回率 | 27.3% | **100.0%** | +72.7% |
| AI召回率（全量） | 98.8% | **99.9%** | +1.1% |
| 人类召回率 | 90.9% | **94.6%** | +3.7% |
| 整体准确率 | 95.2% | **97.7%** | +2.5% |
| HC3回归率 | — | **0%（无回归）** | — |

特别值得注意的是，过采样持续学习未对原领域（HC3问答数据）造成任何性能退化（HC3回归率为0%），证明了本方法在领域自适应中的安全性和有效性。
...
步骤5，评估与验证。在完整测试集（1,406条样本，含原HC3测试数据和教材测试数据）上评估：

- 教材章节p_ai：从过采样前的0.0096提升至0.9980；
- 教材文本召回率：从27.3%提升至100.0%；
- 整体AI召回率：从98.8%提升至99.9%；
- 整体人类召回率：从90.9%提升至94.6%；
- 整体准确率：从95.2%提升至97.7%；
- HC3回归检测：HC3测试集准确率95.0%，与过采样前持平，无性能退化。
...
步骤S1，快速检测步骤：基于所述语言标签，并行执行以下两个子阶段——
   ...
   步骤S1b，语言学风格特征提取与分类子阶段：纯中央处理器执行，无图形处理器依赖，提取与所述语言类型对应的多层级语言学风格特征并输入第二分类器，输出第二AI概率p_ai₂；其中，所述语言学风格特征中的Token概率分布特征直接复用所述步骤S1a缓存的逐Token对数概率数据，无需第二次语言模型前向计算；
...
根据权利要求5所述的方法，其特征在于，经所述过采样持续学习自适应后，目标领域文本的AI概率p_ai从0.0096提升至0.9980以上，目标领域召回率从27.3%提升至100%，整体检测准确率从95.2%提升至97.7%，且原领域检测精度无退化。
...
根据权利要求1所述的方法，其特征在于，还包括：在所述步骤S4之后，当接收到的检测请求包含诊断标志时，输出语言学诊断信息，所述语言学诊断信息包括微观层人类相似度评分、中观层人类相似度评分、宏观层人类相似度评分、最显著人类特征信号列表以及综合人类相似度评分。
