...
**（1）基于统计特征的检测方法。** 利用参考语言模型（Reference LM）计算待检文本的困惑度（Perplexity）、逐Token熵（Token-level Entropy）及其统计量（均值、标准差、突发性Burstiness等），基于"AI文本对参考LM更平滑、更可预测"的假设进行分类。代表方法包括GPTZero、DetectGPT等。此类方法计算速度快（<200ms），但对流畅、正式的AI文本（如GPT-4生成的技术文档）容易产生假阴性。

**（2）基于编码器的检测方法。** 在预训练语言模型（如RoBERTa、DeBERTa）基础上通过LoRA等参数高效微调技术进行二分类训练。此类方法精度较高（ROC-AUC通常在0.95以上），但存在领域漂移问题——在特定数据集（如HC3-Chinese，基于ChatGPT-3.5的2023年问答数据）上训练的模型，对新型LLM（GPT-4、Claude）生成的文本检测能力急剧下降，p_ai可低至0.01以下。
...
第四，**多阶段级联策略缺乏语言特异性条件逻辑**。现有级联管线通常采用统一的提前退出（Early Exit）阈值和一致的冲突仲裁策略，未考虑中文场景下统计阶段对流畅AI文本过度自信、HC3训练模型对现代LLM灾难性失效等特殊问题。
...
第三，提供一种**基于过采样持续学习的领域自适应方法**，在保持对原领域（如HC3问答数据）检测能力的前提下，通过目标领域样本的过采样以极小数据量（百级样本）实现对新领域（如教材文本、技术文档）的快速适配，解决编码器检测模型的领域漂移问题。
...
**（1）语言路由器（Language Router）：** 基于XLM-RoBERTa多语言分类模型的输入文本语言检测模块，输出语言类型标签及其置信度。系统根据检测结果将输入文本路由至中文管线或英文管线。

**（2）第一阶段（Stage 1）：快速检测阶段。** 包含两个并行执行的子阶段：
- **统计特征提取与分类（Stage 1a）：** 利用参考语言模型（英文使用GPT-2-XL，中文使用Wenzhong-GPT2-110M）计算文本的困惑度、平均熵、熵标准差、突发性、最大熵、最小熵6项特征，由XGBoost或逻辑回归分类器输出AI概率p_ai。
- **语言学风格特征提取与分类（Stage 1b）：** 纯CPU执行，无GPU依赖，提取14项多层级语言学特征，由XGBoost分类器输出AI概率p_ai。该阶段直接复用Stage 1a已缓存的逐Token对数概率数据（特征M5/M6），无需第二次LM前向计算。

**（3）第二阶段（Stage 2）：编码器精确分类阶段。** 采用LoRA微调后的编码器模型（英文使用DeBERTa-v3-large，中文使用Chinese-RoBERTa-wwm-ext-large）进行序列分类，输出AI概率p_ai。
...
**英文管线条件逻辑：**
- 若Stage 1（统计+语言学）的置信度 > 0.99（提前退出阈值），直接返回聚合结果，跳过Stage 2和Stage 3；
- 若Stage 1与Stage 2结果一致（均为AI或均为人写），跳过Stage 3，直接对前两阶段进行加权融合；
- 仅当Stage 1与Stage 2冲突时，才执行Stage 3（Binoculars）进行仲裁。

**中文管线条件逻辑（语言特异性设计）：**
- 禁止Stage 1提前退出——中文统计阶段对流畅AI文本存在过度自信问题，必须等待编码器阶段介入；
- 中文仲裁块（ZH Arbitration Block）：当统计阶段判定为"人写"但编码器阶段p_ai ≥ 0.35时，判定统计阶段为假阴性，彻底忽略统计阶段的分类结果，仅使用编码器阶段的输出作为最终结果；
- 强制执行Binoculars阶段——中文场景下HC3训练的编码器模型对现代LLM文本存在灾难性失效风险（p_ai可低至0.01），Binoculars始终作为安全兜底运行；
...
- **M5（Token对数概率偏度）：** 逐Token对数概率分布的三阶标准化矩；
- **M6（高概率Token占比）：** 对数概率大于-1.0（即概率大于约0.368）的Token占全部Token的比例。
...
| 编号 | 特征名称 | 计算方法 | 语言特异设计 |
|------|----------|----------|--------------|
| M1 | 句长突发性 | (标准差-均值)/(标准差+均值) | 通用 |
| M2 | 句长变异系数 | 标准差/均值 | 通用 |
| M3 | 句长基尼系数 | 标准基尼系数 | 通用 |
| M4 | 句法重复率 | 句首Token集合的平均成对Jaccard相似度 | 中文按字符级分词，英文按单词级分词并过滤停用词 |
| M5 | Token对数概率偏度 | 三阶标准化矩 | 跨阶段复用统计特征 |
| M6 | 高概率Token占比 | log_prob > -1.0的Token比例 | 跨阶段复用统计特征 |
| M7 | 模糊语密度 | 模糊语词表命中数/千字符 | 中英文独立词表（英文：appears to/tends to等；中文：似乎/往往/可能等） |
| M8 | 话语模板密度 | AI模板标记命中数/千字符 | 中英文独立词表（英文：furthermore/moreover等；中文：此外/因此/综上等） |
| M9 | 标点风格密度 | 特殊标点（破折号/括号/分号等）计数/千字符 | 中文侧重——、……、；英文侧重em-dash、en-dash、semicolon |

**中观层（Meso, 段落级）——2项特征：**

| 编号 | 特征名称 | 计算方法 |
|------|----------|----------|
| S1 | 段长方差 | log1p(var(段落字符数)) |
| S2 | 段落模板得分 | 基于句首模板标记命中率和连续段落主题相似度的启发式0-1评分 |
...
| D2 | 作者立场密度 | (第一人称代词命中数 + 模糊语命中数)/千字符 | 中英文独立词表（英文：we/our/I等；中文：我们/笔者/本人等） |
| D3 | 可读性指数 | 英文：Flesch Reading Ease公式；中文：负平均句长代理变量 | 中英文计算公式不同 |
...
- 语言学风格：0.10（辅助信息）
- 统计特征：0.10（辅助信息，但中文统计阶段对流畅AI文本可能产生假阴性，权重压低）
...
**步骤1：领域数据收集。** 收集目标领域的小样本标注数据（如98个教材文本片段，来自10个章节，每章节约500-2000字），按训练/验证/测试划分为82/7/9。

**步骤2：过采样扩增。** 对每个划分中的目标领域样本执行10倍过采样（简单复制），使过采样后目标领域样本在训练集中占比约为7.2%。

**步骤3：参数高效微调。** 使用过采样混合数据集（原领域10,510条HC3数据 + 过采样后820条教材数据）进行LoRA微调（LoRA秩r=16，学习率lr=2e-5，训练3个epoch，混合精度bf16）。

**步骤4：验证与回滚保护。** 在完整测试集上验证检测性能，确保原领域检测精度无退化（回滚保护指标），同时目标领域检测精度显著提升。
...
步骤S1：快速检测阶段。统计特征提取器计算6项LM概率特征并缓存逐Token对数概率；语言学特征提取器（CPU）同步计算14项风格特征（复用缓存的Token对数概率计算M5/M6）。两路特征分别由XGBoost分类器输出p_ai。若为英文且统计+语言学融合置信度 > 0.99，提前退出并返回结果。

步骤S2：编码器分类阶段。LoRA微调编码器模型输出p_ai。若为中文且统计阶段判定"人写"但编码器p_ai ≥ 0.35，触发中文仲裁块，仅采用编码器结果返回。若为中文且编码器置信度 > 0.99，提前退出。若为英文且统计与编码器结果一致，跳过Stage 3。

步骤S3：零样本检测阶段。仅在中英文管线中当前两阶段不一致或中文管线强制触发时执行。Binoculars通过交叉熵比值映射为p_ai。

步骤S4：加权集成。根据语言类型的权重配置，对有输出的各阶段p_ai进行归一化加权求和，应用语言特异决策阈值，输出最终分类标签（"AI生成"/"人类撰写"）、置信度和各阶段分解结果。

## 六、有益效果

### 6.1 语言感知级联管线的效果

通过语言特异性条件逻辑，英文管线在90%以上的请求中可在Stage 1-2内高置信返回，平均响应时间低于500ms；中文管线通过强制Binoculars兜底和仲裁块设计，对流畅AI文本的检测可靠性和鲁棒性显著提升，消除了统计阶段过度自信导致的假阴性。
...
- GPT-4o生成的文本在语言学前最难检测（平均p_ai=0.8350），Qwen-2-72B最易检测（平均p_ai=0.9512）。

在HC3中文测试集（1000条样本）上的验证显示：
...
在教材文本领域适配任务上，10倍过采样持续学习的效果如下。

**关键评估方法说明：** 测试集中的教材样本不参与过采样，保持为不重复的唯一样本（9条），避免重复样本导致的评估偏差。此外，对6个完整章节文件（每个截取前3000字符，长度不同于训练时的~1000字符分块）进行了泛化测试。

| 指标 | 过采样前（v1） | 过采样后（v2） | 评估口径 |
|------|:-------------:|:-------------:|---------|
| 教材章节p_ai | 0.0096 ❌ | **0.9980** ✅ | 6个完整章节文件（3000字符） |
| 教材唯一样本召回率 | 0/9 (0%) | **9/9 (100%)** | 9个不重复hold-out样本 |
| 完整章节泛化召回率 | 0/6 (0%) | **6/6 (100%)** | 不同长度的全文测试 |
| HC3 AI召回率 | — | **52/52 (100%)** | 随机100条HC3样本 |
| HC3 human召回率 | — | **45/48 (93.8%)** | 同上 |
| HC3整体准确率 | ~95.2% | **97.0%** | 同上 |

需要指出的是，过采样前的基线模型（v1）对教材内容返回p_ai=0.0096，并非"略有偏差"而是完全失效——该模型在HC3问答数据上训练，无法识别不同风格（正式教材体）的现代LLM文本。过采样的贡献在于：用极少量目标领域数据（82条训练样本×10倍=820条）使模型学会识别该风格，同时不遗忘原有领域知识。

### 6.5 整体系统效果

在多模型集成检测框架下，系统整体准确率达到97.7%（中文测试集1,406条样本），响应时间根据级联路径不同介于200ms至3s之间，支持单GPU（12GB VRAM）环境部署。

### 6.6 已知局限性

**（1）Binoculars零样本检测的边界条件。** Binoculars方法对部分现代LLM生成的中文文本效果有限。在本发明的诊断实验中，Binoculars对GPT-4/Claude风格的教材文本返回的score=0.991，转换后p_ai=0.511——仅略高于随机猜测。这表明当文本生成模型与Binoculars使用的观察者/表演者模型（Qwen2-7B对）能力相近时，交叉困惑度信号的区分力下降。因此，中文管线中Binoculars的作用是补充而非主导，系统的主要检测能力依赖经过领域自适应的编码器。

**（2）过采样方法的适用范围。** 10倍过采样在本实施例中有效，前提条件是：（a）目标领域样本与原领域样本在语言上兼容（同为中文）；（b）目标领域样本数量虽少但质量高（确认为AI生成）；（c）原领域训练数据规模远大于过采样后的目标领域数据（10,510 vs 820），保证原领域知识不被遗忘。若目标领域数据量过大或质量不确定，过采样可能引入噪声。

**（3）领域自适应需要目标领域标注数据。** 本方法不是零样本方法——它需要少量（百级）确认由AI生成的目标领域文本。对于完全未知的领域，仍需人工收集和确认样本。
...
流程图展示了10倍过采样持续学习的完整数据流：HC3-Chinese数据集（10,510样本）与企业AI安全教材（98个文本块）经8:1:1拆分后，对教材训练样本进行10倍过采样（82→820），合并为11,330样本的训练集，进行LoRA微调训练。对比过采样前后：教材章节p_ai从0.0096提升至0.9980，召回率从27.3%提升至100%，HC3无回归。
...
图5展示了过采样持续学习的完整数据流架构：左侧为两类数据源（原领域HC3数据10,510样本 + 目标领域教材98样本），经分层拆分和10倍过采样后合并为11,330样本训练集。LoRA微调产出v2模型后，进入右侧双轨验证：目标领域验证检查教材召回率，原领域验证检查HC3退化率。回归检查门控确保原领域退化低于2%时方可部署，否则拒绝发布。
...
1. 语言路由器 → 中文（置信度0.99）；
2. 统计特征提取（Wenzhong）→ 困惑度23.5，突发性-0.48，p_ai=0.0148（判定人写）；
3. 语言学特征提取（CPU，复用缓存）→ 句长基尼0.35，模糊语密度2.1/千字，词汇多样性MTLD 45.2，p_ai=0.0964（判定人写）；
4. 编码器分类（Chinese-RoBERTa+LoRA v2）→ p_ai=0.9980（判定AI生成）；
5. 触发中文仲裁块（统计p_ai=0.0148判定人写 vs 编码器p_ai=0.9980≥0.35）→ 采用编码器结果；
6. 加权集成（中文权重：编码器0.60 + Binoculars 0.20 + 语言学0.10 + 统计0.10）→ p_ai=0.9980×0.60 + 0.9980×0.20 + 0.0964×0.10 + 0.0148×0.10 = 0.6075（归一化后），决策阈值0.47 → 标签"AI生成"，置信度0.9980。
...
步骤2，过采样扩增。对训练集中的82条教材文本执行10倍过采样（每条复制10次），获得820条训练样本。保持验证集和测试集也执行10倍过采样（分别获得70条和90条），以确保评估时采样代表性。

步骤3，混合训练集构建。将过采样后的教材训练数据（820条）与原有的HC3中文训练数据（10,510条）合并，形成11,330条混合训练集。其中教材数据占比约7.2%。

步骤4，LoRA微调。加载预训练的Chinese-RoBERTa-wwm-ext-large模型，附加LoRA适配器（rank=16，learning_rate=2e-5）。在混合训练集上训练3个epoch，batch_size=16，混合精度bf16。验证集用于早停。

步骤5，评估与验证。在不重复的hold-out测试集上评估（9条不重复教材样本 + 100条随机HC3样本）：

- 教材唯一样本召回率（9条，不重复）：9/9 = 100%；
- 完整章节泛化测试（6个文件，3000字符/个，不同于训练长度）：6/6 = 100%；
- 教材章节p_ai：从0.0096提升至0.9980；
...
- 句长突发性（M1）：人类撰写的学术论文句长波动大（burstiness > 0），AI文本句长更为均匀（burstiness < 0）；
...
步骤4，集成判决。语言学p_ai（权重0.85）与统计p_ai（权重0.15）加权融合。由于编码器和Binoculars在本实施例中禁用（权重为0），系统直接返回两阶段融合结果。对于典型的AI学术论文，综合p_ai > 0.50即可判定为AI生成。
...
步骤S0，语言检测步骤：将输入文本经语言路由器处理后，判定其语言类型并输出语言标签；

步骤S1，快速检测步骤：基于所述语言标签，并行执行以下两个子阶段——
   步骤S1a，统计特征提取与分类子阶段：利用与所述语言类型对应的参考语言模型，计算输入文本的统计特征并输入第一分类器，输出第一AI概率p_ai₁，同时缓存逐Token对数概率数据；
   步骤S1b，语言学风格特征提取与分类子阶段：纯中央处理器执行，无图形处理器依赖，提取与所述语言类型对应的多层级语言学风格特征并输入第二分类器，输出第二AI概率p_ai₂；

步骤S2，编码器分类步骤：利用与所述语言类型对应的经参数高效微调的编码器模型对输入文本进行分类，输出第三AI概率p_ai₃；

步骤S3，零样本检测步骤：利用与所述语言类型对应的双模型比对方法检测输入文本，输出第四AI概率p_ai₄；
...
（a）当所述语言类型为英文时，若所述步骤S1输出的融合置信度大于0.99的提前退出阈值，则直接进入所述步骤S4，跳过步骤S2和S3；若所述步骤S1与步骤S2的分类结果一致，则跳过步骤S3，由步骤S4直接融合前两阶段结果；

（b）当所述语言类型为中文时，禁止所述步骤S1的提前退出；当所述步骤S1a输出的第一AI概率p_ai₁低于0.50且所述步骤S2输出的第三AI概率p_ai₃大于等于0.35时，触发中文仲裁块，所述步骤S4仅采用所述步骤S2的结果作为输入；所述步骤S3始终执行，不因前两阶段结果一致而跳过；
...
步骤E，验证与回滚保护：在同时包含原领域测试数据和目标领域测试数据的完整测试集上验证微调后模型的检测性能，当目标领域检测精度达到预设标准且原领域检测精度无退化时，部署更新后的编码器模型。

### 从属权利要求6

根据权利要求5所述的方法，其特征在于，所述N=10；所述目标领域为教材文本领域；所述原领域数据集为HC3-Chinese问答数据集；所述编码器模型为基于Chinese-RoBERTa-wwm-ext-large的LoRA微调模型；所述微调参数为LoRA秩r=16，学习率lr=2e-5，训练3个epoch，混合精度bf16。

### 从属权利要求7

根据权利要求5所述的方法，其特征在于，经所述过采样持续学习自适应后，在9个不重复的目标领域hold-out测试样本上达到100%召回率，在6个完整目标领域文档（长度不同于训练样本）上同样达到100%召回率，目标领域文本的AI概率p_ai从0.0096提升至0.9980以上，且在原领域100条随机抽样测试中AI召回率保持100%、人类召回率不低于93%。
